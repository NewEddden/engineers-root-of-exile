...
(function () {
  ...
  function render() {
    ...
    // count line
    if (query) {
      countEl.textContent = total + (total === 1 ? ' chapter' : ' chapters') +
        ' match "' + query + '".';
    } else {
      countEl.textContent = ALL.length + ' chapters posted. Page ' + page + ' of ' + maxPage + '.';
    }
    ...
    if (rows.length === 0) {
      ...
      li.innerHTML = '<div class="toc-empty">No chapters match that search.</div>';
      ...
      for (const ch of rows) {
        ...
        a.innerHTML =
          '<span class="num">Chapter ' + ch.n + '</span>' +
          '<span class="ttl">' + escapeHtml(ch.title) + '</span>' +
          '<span class="meta">' + EROE.fmtDate(ch.date) +
          (ch.minutes ? ' &middot; ' + ch.minutes + ' min read' : '') + '</span>';
        li.appendChild(a);
        listEl.appendChild(li);
      }
    }
  ...
  window.addEventListener('popstate', () => { readUrl(); inputEl.value = query; applyFilter(); render(); });

  EROE.load().then(({ chapters }) => {
    ALL = chapters;
    readUrl();
    inputEl.value = query;
    applyFilter();
    ...
    countEl.textContent = '';
    listEl.innerHTML = '<li><div class="toc-empty">' + err.message +
